...
struct Registers {
	...
	void setRegister(string name, int value) {
		regs[name] = value;
		if(regs[name] > highestValue.get(name, 0)) {
			highestValue[name] = regs[name];
		}
	}
	...
	Tuple!(string, int)[] highestValues() {
		Tuple!(string, int)[] items;
		foreach(k, v; highestValue) {
			items ~= tuple(k, v);
		}
		sort!((a ,b) { return a[1] > b[1]; })(items);
